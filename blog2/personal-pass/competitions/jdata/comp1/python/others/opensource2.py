...
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# github https://github.com/onion1003/JData_2018/blob/master/mybaseline0.3158.py
...
sku = pd.read_csv(data_root + 'jdata_sku_basic_info.csv', )
action = pd.read_csv(data_root + 'jdata_user_action.csv', parse_dates=['a_date'])
basic_info = pd.read_csv(data_root + 'jdata_user_basic_info.csv')
comment_score = pd.read_csv(data_root + 'jdata_user_comment_score.csv', parse_dates=['comment_create_tm'])
order = pd.read_csv(data_root + 'jdata_user_order.csv', parse_dates=['o_date'])
...
def make_user_order_table():
    ...
    # train_data = order[order['month'] == train_month][['user_id', 'o_date', 'cate']]
    # .sort_values(by=['user_id', 'o_date']).drop_duplicates()
    ...
    train.to_csv('./processed_data/user_order_table.csv', index=False)
    print(train.head())
...
if __name__ == '__main__':
    ...
    new_order = pd.read_csv('./processed_data/user_order_table.csv', parse_dates=['o_date'])
    ...
    new_order = new_order.sort_values(by=['year_user_orders',
                                          'mon_4_user_orders',
                                          'mon_3_user_orders',
                                          'mon_2_user_orders',
                                          'mon_1_user_orders'],
    ...
    print(np.sum((new_order['mon_4_user_orders'][:50000] +
                  new_order['mon_3_user_orders'][:50000] +
                  new_order['mon_2_user_orders'][:50000] +
                  new_order['mon_1_user_orders'][:50000]) == 0))
    # 用前15天随机出时间
