...
def get_target_txt_file(image_file):
    #
    pre_dir = os.path.abspath(os.path.dirname(image_file) + os.path.sep + "..")
    text_dir = os.path.join(pre_dir, 'contents')
    #
    file_name = os.path.basename(image_file)
    arr_split = os.path.splitext(file_name)
    file_name = arr_split[0] + '.txt'
    #
    text_file = os.path.join(text_dir, file_name)
...
def get_list_contents(content_file):
    ...
    for line in lines:
        arrs = line.split('|')
        item = list(map(lambda x: int(x), arrs[0].split('-')))

        contents.append([item, arrs[1]])
    return contents
...
def overlap(h_up1, h_dw1, h_up2, h_dw2):
    overlap_value = (min(h_dw1, h_dw2) - max(h_up1, h_up2)) / (max(h_dw1, h_dw2) - min(h_up1, h_up2))
