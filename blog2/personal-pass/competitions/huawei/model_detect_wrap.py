...
class ModelDetect:
    ...
    def create_graph_all(self, training):
        ...
        with self.graph.as_default():
            ...
            self.rnn_cls, self.rnn_ver, self.rnn_hor = model_def.rnn_detect_layers(self.conv_feat, self.seq_len,
                                                                                   len(meta.anchor_heights))
            #
            ...
            #
            self.loss = model_def.detect_loss(self.rnn_cls, self.rnn_ver, self.rnn_hor, self.t_cls, self.t_ver,
                                              self.t_hor)
            #
            # print(loss.op.name)
    ...
    def train_and_valid(self, data_train, data_valid):
        # model save-path
        if not os.path.exists(meta.model_detect_dir):
            os.mkdir(meta.model_detect_dir)
